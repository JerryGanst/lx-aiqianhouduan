<template>
  ...
  >
    <div class="upload-layout">
      <!-- 左侧附件列表 -->
      <div class="file-list" :style="{ width: isPre ? '780px' : '100%' }" ref="leftPanel">
...
import { TABLE_AGENT_TYPE } from '@/utils/constants.js'
...
const { limitFile, limitFinalFile, currentId, currentAgentType } = useShared()
  ...
  request
    ...
    .then(res => {
      if (res.status) {
        ...
        if (fileQueue.value && fileQueue.value.length > 0 && currentAgentType.value === TABLE_AGENT_TYPE) {
          fileQueue.value = fileQueue.value.filter(item => {
            return ['xlsx', 'xls'].includes(item.fileType)
          })
        }
